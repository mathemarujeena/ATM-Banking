...
class UserRegistrationView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if registration_form.is_valid() and address_form.is_valid() and information_form.is_valid():
                ...
                information = information_form.save(user,commit=False)
    ...
    def get_context_data(self, **kwargs):
        if 'registration_form' not in kwargs:
            kwargs['registration_form'] = UserRegistrationForm()
        if 'information_form' not in kwargs:
            kwargs['information_form'] = UserInformationForm()
        if 'address_form' not in kwargs:
            kwargs['address_form'] = UserAddressForm()
        return super().get_context_data(**kwargs)
...
def UserRegistrationView(request):
    if request.method == 'POST':
        ...
        if registration_form.is_valid() and address_form.is_valid() and information_form.is_valid():
            ...
            information = information_form.save(user,commit=False)
            information.user = user
            information.fingerprint_images= request.FILES['fingerprint_images']
            information.account_no=(
                user.id +
                settings.ACCOUNT_NUMBER_START_FROM
            )
        ...
        address_form = UserAddressForm()
        information_form = UserInformationForm()
    context_dict = {'registration_form': registration_form,'address_form':address_form,'information_form':information_form }
